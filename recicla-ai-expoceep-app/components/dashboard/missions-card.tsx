        ...
        >
          {missions.map((mission, index) => (
            <motion.div
              ...
              className="flex flex-col gap-2 retro-border-item retro-shadow-sm retro-radius bg-card px-4 py-3"
            >
              ...
              {mission.completed ? (
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="h-2 bg-muted overflow-hidden retro-border-item retro-radius">
                    <div
                      ref={(el: HTMLDivElement | null) => { if (el) progressRefs.current[index] = el; }}
                      data-target={`${(mission.current / mission.target) * 100}%`}
                      className="h-full bg-success retro-radius"
                      style={{ width: "0%" }}
                    />
                  </div>
                  <span className="text-xs text-muted-foreground">
